...
def extraction_relations(record):
    ...
    file = open("parent.txt").readlines()
    has_parent_relations = []
    for line in file:
        has_parent_relations.append(line.split(" ")[0])
    # Extract DateOfBrith relations.
    results.extend(extract_date_of_birth(record['sentence']))
    #print(results,record['sentence_id'])

    # Extract Parent relations.
    results.extend(extract_has_parent(record['sentence']))
    
    #if results == [] and record['sentence_id'] in has_parent_relations :
    #    print('not match@@@@@@@@@@',results,record['sentence_id'])
    #if results != [] and record['sentence_id'] not in has_parent_relations:
    #    print('over match!!!!!',results,record['sentence_id'])
    return results
